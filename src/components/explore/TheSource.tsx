...
import { ArrowRight, ArrowLeft, HelpCircle, User, Users, Sparkles, Check, Brain, BookOpen, MessageSquare, Clock, Heart, AlertTriangle, Shield, Globe } from 'lucide-react';

interface TheSourceProps {
...
export const TheSource = ({ verifiedCount, totalFacts, onComplete, onBack }: TheSourceProps) => {
  const [stage, setStage] = useState<Stage>('question');
  const [selectedOption, setSelectedOption] = useState<string | null>(null);
  const [showReflection, setShowReflection] = useState(false);
  const [showSpecialistLogic, setShowSpecialistLogic] = useState(false);
  const [currentVoiceScene, setCurrentVoiceScene] = useState(0);
  ...
  // Voice scene navigation
  const voiceScene = voiceScenes[currentVoiceScene];
  const isLastVoiceScene = currentVoiceScene === voiceScenes.length - 1;
  const isFirstVoiceScene = currentVoiceScene === 0;

  const handleVoiceNext = () => {
    if (isLastVoiceScene) {
      onComplete();
    } else {
      setCurrentVoiceScene(prev => prev + 1);
    }
  };

  const handleVoiceBack = () => {
    if (isFirstVoiceScene) {
      setStage('question');
    } else {
      setCurrentVoiceScene(prev => prev - 1);
    }
  ...
  return (
    ...
    >
      {/* Progress dots - positioned below the main progress bar */}
      <div className="fixed top-24 md:top-20 left-1/2 -translate-x-1/2 flex items-center gap-1.5 z-40 bg-slate-900/80 backdrop-blur-sm px-3 py-2 rounded-full border border-slate-700/50">
        {voiceScenes.map((_, i) => (
          <button
            key={i}
            onClick={() => setCurrentVoiceScene(i)}
      ...
      <div className="flex-1 flex items-center justify-center p-4 pt-16">
        <div className="max-w-2xl w-full">
          <AnimatePresence mode="wait">
            ...
            >
              {/* Scene Icon */}
              <div className={`w-24 h-24 ${voiceScene.iconBg} rounded-full flex items-center justify-center mx-auto mb-6`}>
                <div className={voiceScene.iconColor}>
                  {voiceScene.icon}
                </div>
              </div>
              ...
              <p className="text-slate-500 text-sm mb-8">
                {currentVoiceScene + 1} of {voiceScenes.length}
              </p>
              ...
              <div className="flex items-center justify-center gap-4">
                ...
                >
                  {isLastVoiceScene ? 'Continue' : 'Next'}
                  <ArrowRight className="w-5 h-5" />
                </button>
              </div>
            </motion.div>
