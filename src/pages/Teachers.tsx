    ...
    try {
      ...
      const teachersData = profilesData
        ...
        .map((teacher: any) => {
          ...
          const independentRate = teacher.independent_rate ? parseFloat(teacher.independent_rate) : 0;
          return {
            id: teacher.id,
            user_id: teacher.user_id,
            bio: teacher.bio,
            hourly_rate: isIndependent ? independentRate : (tierStats?.teacher_hourly_rate || 0),
            ...
            student_hourly_price: isIndependent ? independentRate : (tierStats?.student_hourly_price || 15),
            ...
            payment_collection: teacher.payment_collection || undefined,
          };
        }) || [];
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1">
              {loading ? (
                ...
                <>
                {(() => {
                  const platformTeachers = paginatedTeachers.filter(t => t.teacher_type !== 'independent');
                  const independentTeachers = paginatedTeachers.filter(t => t.teacher_type === 'independent');
                    ...
                    <div className="space-y-2">
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <button
                          ...
                          className={`px-4 py-2.5 bg-gradient-to-r ${teacher.teacher_type === 'independent' ? 'from-slate-600 to-indigo-600' : getTierColor(teacher.tier)} text-white rounded-lg font-semibold transition hover:opacity-90 shadow-md`}
                        >
                          Book Now
                        </button>
                      </div>
                  ...
                  const renderTeacherInfo = (teacher: Teacher) => (
                    <>
                      <div className="flex items-start space-x-4 mb-4">
                  ...
                  return (
                    <>
                      {/* Platform Teachers Section */}
                      {platformTeachers.length > 0 && (
                        <>
                          {independentTeachers.length > 0 && (
                            <div className="mb-6">
                              <h2 className="text-xl font-bold text-gray-900 dark:text-white">Our Teachers</h2>
                              <p className="text-sm text-gray-500 dark:text-gray-400">Platform teachers with structured pricing tiers</p>
                            </div>
                          )}
                          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                            {platformTeachers.map((teacher) => (
                              <div
                                key={teacher.id}
                                    ...
                                    <div className="flex items-center space-x-2">
                                      {teacher.rating_avg > 0 && (
                                        <div className="flex items-center space-x-1">
                                          ...
                                          <span className="text-sm font-semibold text-gray-700">{teacher.rating_avg.toFixed(1)}</span>
                                          <span className="text-xs text-gray-500">({teacher.rating_count})</span>
                                        </div>
                                      )}
                                      {teacher.total_feedback > 0 && (
                                        <div className="flex items-center space-x-1 bg-green-500 px-2 py-0.5 rounded-full">
                                          <ThumbsUp className="w-3 h-3 text-white" />
                                          <span className="text-xs font-bold text-white">{teacher.thumbs_up_percentage}%</span>
                                        </div>
                                      )}
                                    </div>
                                  </div>
                      ...
                      {/* Independent Teachers Section */}
                      {independentTeachers.length > 0 && (
                        <>
                          <div className={platformTeachers.length > 0 ? 'mt-12 mb-6' : 'mb-6'}>
                            <h2 className="text-xl font-bold text-gray-900 dark:text-white">Independent Teachers</h2>
                            <p className="text-sm text-gray-500 dark:text-gray-400">Experienced teachers who manage their own pricing</p>
                          </div>
                          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                            {independentTeachers.map((teacher) => (
                              <div
                                key={teacher.id}
                                className="bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-all duration-300"
                              >
                                <div className="px-4 py-2 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                                  <div className="flex items-center justify-between">
                                    <div className="flex items-center space-x-2">
                                      <span className="px-2 py-0.5 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 text-xs font-semibold rounded-full">Independent</span>
                                    </div>
                                    <div className="flex items-center space-x-2">
                                      {teacher.rating_avg > 0 && (
                                        <div className="flex items-center space-x-1">
                                          <Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
                                          <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">{teacher.rating_avg.toFixed(1)}</span>
                                          <span className="text-xs text-gray-500">({teacher.rating_count})</span>
                                        </div>
                                      )}
                                      {teacher.total_feedback > 0 && (
