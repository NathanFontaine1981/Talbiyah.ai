...
export default function TeacherAgreement() {
  ...
  // When already accepted, the page becomes a read-only review instead of redirecting.
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Already accepted the current version → show a read-only review (so teachers
        // can revisit the agreement any time from the hub) instead of redirecting away.
        if (profile.agreement_accepted_at && profile.agreement_version === AGREEMENT_VERSION) {
          setAcceptedAt(profile.agreement_accepted_at);
          setChecked(new Set(AGREEMENT_SECTIONS.map((s) => s.id)));
        }
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-2">
            {acceptedAt
              ? 'You have completed this agreement. Here it is for your reference.'
              : 'Please read and confirm each section before you begin teaching.'}
          </p>
        </div>

        {/* Accepted banner (review mode) */}
        {acceptedAt && (
          <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-2xl p-4 mb-6">
            <CheckCircle className="w-6 h-6 text-emerald-600 flex-shrink-0" />
            <p className="text-sm text-emerald-800">
              <span className="font-semibold">Accepted</span> on{' '}
              {new Date(acceptedAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}.
              You can re-read every section below at any time.
            </p>
          </div>
        )}
            ...
            return (
              ...
              >
                <div className="p-6">
                  ...
                  <label className={`mt-2 flex items-center gap-3 select-none rounded-lg p-3 border transition-colors ${
                    acceptedAt ? 'cursor-default' : 'cursor-pointer'
                  } ${
                    ...
                    <input
                      ...
                      className="w-5 h-5 text-emerald-600 border-gray-300 rounded focus:ring-emerald-500 disabled:opacity-100"
                    />
        ...
        {/* Review mode: just a way back. Sign mode: signature + confirm. */}
        {acceptedAt ? (
          <div className="mt-6 text-center">
            ...
            >
              <CheckCircle className="w-5 h-5" /> Back to Teacher Hub
            </button>
          </div>
        ) : (
