...
async function getRecordingAssets() {
  ...
  const recordingId = '692c0b7dcdada8de847b5e69';
  
  console.log('Fetching recording assets for:', recordingId);
  ...
  console.log('\nRecording assets:', JSON.stringify(data, null, 2));
    ...
    if (transcriptAsset) {
      ...
      if (urlData.url) {
        ...
        if (contentType?.includes('json')) {
          ...
          console.log('\n--- TRANSCRIPT JSON ---');
          console.log(JSON.stringify(transcript, null, 2).substring(0, 10000));
        } else {
          const transcript = await transcriptResponse.text();
          console.log('\n--- TRANSCRIPT TEXT ---');
          console.log(transcript.substring(0, 10000));
        }
      }
    }
  }
